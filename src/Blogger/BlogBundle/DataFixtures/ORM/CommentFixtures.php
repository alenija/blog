...
class CommentFixtures extends AbstractFixture implements OrderedFixtureInterface
{
    public function load(ObjectManager $manager)
    {
        ...
        $comment->setUser('symfony')
            ->setComment('To make a long story short. You can\'t go wrong by choosing Symfony! And no one has ever been fired for using Symfony.')
            ->setBlog($manager->merge($this->getReference('blog-1'))); //'blog-1' - ссылки на сущности Blog кот. созд. ранее в BlogFixtures.php
        $manager->persist($comment);

        $comment = new Comment();
        $comment->setUser('David')
            ->setComment('To make a long story short. Choosing a framework must not be taken lightly; it is a long-term commitment. Make sure that you make the right selection!')
        ...
        $comment = new Comment();
        $comment->setUser('Dante')
            ->setComment('Oh, come on, Stan. Not everything ends the way you think it should. Besides, audiences love happy endings.')
            ->setBlog($manager->merge($this->getReference('blog-6')));
        $manager->persist($comment);

        $comment = new Comment();
        $comment->setUser('Kep')
            ->setComment('Not everything ends the way you think it should.')
            ->setBlog($manager->merge($this->getReference('blog-7')));
        $manager->persist($comment);

        $comment = new Comment();
        $comment->setUser('Thinker')
            ->setComment('Besides, audiences love happy endings.')
            ->setBlog($manager->merge($this->getReference('blog-8')));
        $manager->persist($comment);

        $comment = new Comment();
        $comment->setUser('Ironman')
            ->setComment('Oh, come on, Stan. Not everything ends the way you think it should. Besides, audiences love happy endings.')
            ->setBlog($manager->merge($this->getReference('blog-9')));
        $manager->persist($comment);

        $comment = new Comment();
        $comment->setUser('Halk')
            ->setComment('Not everything ends the way you think it should. Besides, audiences love happy endings.')
            ->setBlog($manager->merge($this->getReference('blog-10')));
        $manager->persist($comment);
